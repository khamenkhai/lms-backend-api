  ...
  try {
    ...
    const enriched = modules.map((module) => {
      ...
      const completedCount = contents.filter(
        (c) => c.userContentProgresses[0]?.is_completed
      ).length;
      ...
      return {
        ...
        progress: {
          totalContents,
          completedCount,
          progressPercentage: parseInt(progressPercentage.toString(), 10),
        },
      };
    });

    sendResponse(res, 200, "Course Modules fetched successfully!", enriched);
  } catch (error) {
...
export const completeModuleByStudent = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (!enrollment) {
      throw new AppError(
        "You are not enrolled in the course for this module.",
        401
      );
    }

    const existingModuleProgress =
      await prismaClient.userModuleProgress.findUnique({
        where: {
          user_id_module_id: {
            user_id: userId,
            module_id: moduleId,
